...
1. A scheduled workflow (daily) and manual `workflow_dispatch` clone the upstream Mercurial
   repo and detect every changeset that does **not** yet have a release.
2. **Every unbuilt changeset is built** (not just the latest), so multiple commits in a day
   are all covered. A per-run cap prevents runaway backfills; capped revisions are picked up
   on later runs (and logged).
